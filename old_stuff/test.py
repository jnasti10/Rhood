...
cur_data = load_data()
cur_data.current_inv = float(c.get_crypto_positions('cost_bases')[1][0]['direct_cost_basis'])
#print(cur_data.current_inv)
...
if(last_hour[0] < last_hour[-1]):
    ...
    print(f'  price increased, p = {p}')
    if(p > cur_data.total * 0.01 and p >= 100):
        print(f'    buying {p} BTC')
        order_info = buy_price('BTC', p)
        if(order_info):
            cur_data.update_data(order_info)
else:
    p = cur_data.current_inv * 0.95
    print(f'  price decreased, p = {p}')
    if(p > cur_data.total * 0.01 and p >= 100):
        print(f'    selling {p} BTC')
        order_info = sell_price('BTC', p)
        if(order_info):
            cur_data.update_data(order_info)

#o = buy_price('BTC', 49)
#if(o):
#    cur_data.update_data(o)
#print(o)
